...
public class SpotifyService {
    ...
    public String setSpotifyToken(Long userId, String token, String refresh) throws UserNotFoundException, Exception {
        try {
            ...
            SpotifyToken userToken = user.get().getSpotifyToken();
            ...
            if (userToken == null) {
                ...
                userToken.setExpires(LocalDateTime.now().plusHours(1));

                userToken = spotifyTokenRepository.save(userToken);
                userRepository.save(user.get().setSpotifyToken(userToken));
